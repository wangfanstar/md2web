...
- `html/index_all.html` 文件夹视图：正文只显示当前文件夹的文档与子文件夹（含大小/更新时间），不显示文档内容与本文目录；在条目上右键可新建文档/新建文件夹/重命名/删除（移入 `data/trash` 可找回）与设置分组（管理员）。
- 多仓库站点：`docs/index.html` 是唯一留在 `docs` 根目录的总览页（按分组列出各文件夹入口 + 跨仓库搜索），其余页面与数据都在 `docs/html/`；**每个 `docs/md` 一级文件夹都有独立入口页** `html/index_<文件夹或仓库名>.html`（独立侧栏与搜索索引，已配置的仓库可标记只读/禁止合入，页面里的「返回首页」默认回到 `html/index_all.html`），`html/index_all.html` 为全部文档合并视图，进入某文件夹后左侧只显示该文件夹的目录、右侧显示当前文档目录。仓库在 `html/md2web_config.html` 配置页维护：默认只显示文件夹名、入口页链接、最新更新（作者/时间）与仓库大小（合计/文档/附件/子文件夹分类，可手动「刷新文件夹信息」），勾选「配置 SVN」后才展开 SVN 地址、更新频率、分组、只读/允许合入等设置（可一键「创建并拉取」自动建目录并下载 SVN 内容）。
