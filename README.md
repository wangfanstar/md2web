...
- 阅读增强：复制路径、查看源码（免登录，可编辑/预览/保存到本地）、双栏编辑器（Markdown 格式分色 + 实时预览，左侧固定大纲导航（默认显示，点击章节时右侧预览同步滚动）、支持撤销/恢复）、`Ctrl+S` 保存草稿或直连写回、下载 MD、下载本页为自包含 HTML（目录固定左侧）、序号开关
- 数学公式：`$...$` / `$$...$$` 由 KaTeX 离线渲染（编辑器预览与文档页一致）
- 提交同时存档图片：编辑器里粘贴到文档同级 `images/` 的图片会随文档一起提交到 SVN（审阅面板会列出将一并存档的图片；设置界面按 AI 助手 / SVN 与仓库 / 账号 / 信息查询 分类显示）。
- `index_all.html` 文件夹视图：正文只显示当前文件夹的文档与子文件夹（含大小/更新时间），不显示文档内容与本文目录；在条目上右键可新建文档/新建文件夹/重命名/删除（移入 `data/trash` 可找回）与设置分组（管理员）。
- 多仓库站点：`index.html` 是按分组的总览页；每个仓库有独立入口页 `index_<仓库名>.html`（独立侧栏与搜索索引，可标记只读/禁止合入，页面里的「返回首页」默认回到 `index_all.html`）；`index_all.html` 为合并视图，首页按分组列出各仓库入口，进入某文件夹后左侧只显示该文件夹的目录、右侧显示当前文档目录。
- 多仓库站点：`index.html` 是按分组的总览页（列出各文件夹入口与跨仓库搜索）；**每个 `docs/md` 一级文件夹都有独立入口页** `index_<文件夹或仓库名>.html`（独立侧栏与搜索索引，已配置的仓库可标记只读/禁止合入），`index_all.html` 为全部文档合并视图；仓库在 `md2web_config.html` 配置页维护：默认只显示文件夹名、入口页链接、最新更新（作者/时间）与仓库大小（合计/文档/附件/子文件夹分类，可手动「刷新文件夹信息」），勾选「配置 SVN」后才展开 SVN 地址、更新频率、分组、只读/允许合入等设置（可一键「创建并拉取」自动建目录并下载 SVN 内容）。
- 仓库同步凭据与网站数据备份：仓库配置页可为每个仓库（及网站备份）设置**同步用户名/密码**（加密入库，优先于环境变量）；「网站数据备份（SVN）」可把 `docs/` 下的网站数据按频率定时合入指定 SVN 库，也可点「立即备份」
...
6. 用「复制链接」复制当前页面链接（含 #/ 路由，可直接分享），用代码块「复制」取出命令；阅读宽表格或长代码时开启「宽屏阅读」。
...
- 读者反馈：`md2web_feedback.html`（每个页面侧栏「反馈」链接可直达）——登录账号的读者可提交问题，未登录只能查看；管理员可更新处理进度（待处理/处理中/已解决/已关闭）与处理说明
