...
- 阅读增强：复制路径、查看源码（免登录，可编辑/预览/保存到本地）、双栏编辑器（Markdown 格式分色 + 实时预览，左侧固定大纲导航（默认显示，点击章节时右侧预览同步滚动），预览弹窗采用布局居中避免文字发虚）、支持查找/替换（`Ctrl+F` / `Ctrl+H`，大小写/整词/正则、计数与全部替换，均可撤销）、撤销/恢复、`Ctrl+S` 保存（无 SVN 库=保存到服务器；有 SVN 库=本地暂存后再提交 SVN）、下载 MD、下载本页为自包含 HTML（目录固定左侧）、序号开关；工具栏可「上传图片 / 上传附件」，粘贴或拖入图片自动上传到文档同级 `images/`，拖入其他文件上传到文档同级 `附件/` 并插入链接，`Ctrl+Alt+K` 文字颜色在预览与站点中均生效
...
- `html/index_all.html` 与每个 `html/index_<仓库>.html` 都提供对应目录的 Markdown 文件列表（含递归子目录、大小/更新时间；来自子文件夹的文档按文件夹分组显示，分组标题里是文件夹路径与配置分组）；列表按钮、正文右键和左侧导航右键共用新建文档/新建文件夹/重命名/**移动到其他文件夹**/删除/设置分组（左侧导航的分组标签带真实目录，空文件夹也会显示，可直接右键在其中新建文档或子文件夹；移动会带上文档引用的 `images/`、`附件/`，跨仓库不允许）。关联 SVN 的操作先通过独立工作副本提交成功再发布本地，重命名/移动保留 SVN 历史，失败或冲突不会误改本地；删除的文档移入对应仓库的 `回收站/`，可恢复或清空。
...
- 保存语义（编辑器 `Ctrl+S`）：**没有 SVN 库**的文档（未关联仓库或本地模式）直接「保存到服务器」——写入 `docs/md` 并重建站点，关闭编辑器后刷新即可看到更新；**有 SVN 库**的文档「保存」只做**本地暂存**（带版本的草稿，支持修改历史、已发布↔草稿差异、放弃草稿，冲突只提示不覆盖），再用「提交 SVN」写入仓库。离开页面前若还有未提交的本地暂存（或编辑器里有未保存修改），浏览器会弹确认提醒
