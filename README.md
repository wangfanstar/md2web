...
- `html/index_all.html` 与每个 `html/index_<仓库>.html` 都提供对应目录的 Markdown 文件列表（含递归子目录、大小/更新时间）；列表按钮、正文右键和左侧导航右键共用新建文档/新建文件夹/重命名/删除/设置分组。关联 SVN 的操作先通过独立工作副本提交成功再发布本地，重命名保留 SVN 历史，失败或冲突不会误改本地；本地文件删除移入 `data/trash` 可找回。
