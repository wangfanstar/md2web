...
- 阅读增强：复制路径、查看源码（免登录，可编辑/预览/保存到本地）、双栏编辑器（Markdown 格式分色 + 实时预览，左侧固定大纲导航（默认显示，点击章节时右侧预览同步滚动）、支持撤销/恢复）、`Ctrl+S` 保存草稿或直连写回、下载 MD、下载本页为自包含 HTML（目录固定左侧）、序号开关；工具栏可「上传图片 / 上传附件」，粘贴或拖入图片自动上传到文档同级 `images/`，拖入其他文件上传到文档同级 `附件/` 并插入链接，`Ctrl+Alt+K` 文字颜色在预览与站点中均生效
