...
- 多仓库站点：`index.html` 是按分组的总览页（列出各仓库入口与跨仓库搜索）；每个仓库有独立入口页 `index_<仓库名>.html`（独立侧栏与搜索索引，可标记只读/禁止合入），`index_all.html` 为全部文档合并视图；仓库在 `md2web_config.html` 配置页维护（地址、目录映射、更新频率、分组、只读/允许合入，可一键「创建并拉取」自动建目录并下载 SVN 内容）。
