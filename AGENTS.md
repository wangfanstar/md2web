...
- `start_linux.sh` 默认**后台启动**（`nohup … >> data/serve.log 2>&1 &`，等待就绪后打印 PID/日志/停止命令），`--foreground` 前台、`--stop` 按 pidfile 停止、`--status` 查看状态；脚本自身的开关会先剥离，其余参数透传 `serve.py`；后台模式自动补 `--no-browser`；保持 POSIX sh、LF 与可执行位。
