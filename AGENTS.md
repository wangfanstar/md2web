...
- 读者反馈：`web/md2web_feedback.html` + `web/md2web-feedback.js`（构建复制到 `docs/` 与 `docs/lib/`）：`GET /__feedback`（公开只读，含状态字典）、`POST /__feedback`（登录 + CSRF，标题≥2 字 + 描述）、`POST /__admin/feedback`（管理员 + CSRF，更新 status/note）；数据库 v7 `feedback` 表；每个页面侧栏（custom-search 顶部行）与总览/配置页都有「反馈」链接。
